...
class player_brain_v5(nn.Module):
    def __init__(
        self,
    ):
        super().__init__()
    ...
    def save(self, working_dir: str, reinforcement=False) -> None:
        if reinforcement:
            torch.save(
                self.state_dict(),
                os.path.join(working_dir, "player_brain_reinforced_v5.pt"),
            )
        else:
            torch.save(
                self.state_dict(),
                os.path.join(working_dir, "player_brain_supervised_v5.pt"),
            )

    def load(self, working_dir, reinforcement=False):
        if reinforcement:
            self.load_state_dict(
                torch.load(os.path.join(working_dir, "player_brain_reinforced_v5.pt"))
            )
        else:
            self.load_state_dict(
                torch.load(os.path.join(working_dir, "player_brain_supervised_v5.pt"))
            )


# v6
class player_brain_v6(nn.Module):
    def __init__(
        self,
    ):
        super().__init__()
        ...
        self.after_lstm_layer_clue = nn.Sequential(nn.Linear(128, 128), nn.Tanh())
        self.pooling = torch.nn.AdaptiveAvgPool1d(1)
        ...
        self.classifier = nn.Sequential(
            nn.Tanh(), nn.Linear(self.classifier_input_size, 26)
        )
    ...
    def forward(self, clue_tensor, length, guessed_tensor):
        ...
        lstm1_out, (h_n, c_n) = self.lstm1(embedded_clue)
        lstm2_out, (h_n, c_n) = self.lstm2(lstm1_out)
        clue_embedded = self.after_lstm_layer_clue(
            self.pooling(lstm2_out.transpose(1, 2)).squeeze(-1)
        )
        guess_embedded = self.guessed_embedding(guessed_tensor)
        x = torch.cat(
    ...
    def save(self, working_dir: str, reinforcement=False) -> None:
        if reinforcement:
            torch.save(
                self.state_dict(),
                os.path.join(working_dir, "player_brain_reinforced_v6.pt"),
            )
        else:
            torch.save(
                self.state_dict(),
                os.path.join(working_dir, "player_brain_supervised_v6.pt"),
            )

    def load(self, working_dir, reinforcement=False):
        if reinforcement:
            self.load_state_dict(
                torch.load(os.path.join(working_dir, "player_brain_reinforced_v6.pt"))
            )
        else:
            self.load_state_dict(
                torch.load(os.path.join(working_dir, "player_brain_supervised_v6.pt"))
            )
